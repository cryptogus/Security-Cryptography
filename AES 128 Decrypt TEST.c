...
unsigned char mix_e(unsigned char b) {
	unsigned char x;
	x = mix8(b) ^ mix4(b) ^ mix2(b);
	return x;
}
void AES(unsigned char plainText[16], unsigned char cipherText[16], unsigned char key[16])
{
...
void printArray(unsigned char input[16]);
...
int main()
{
	...
	AES(userPlainText, state, key);

	if (!compare("AES_decrypt", userPlainText, plainText))
	{
		system("pause");
		return 1;
	}
	printf("userPlainText : ");
	for (int i = 0; i < 16; i++) {
		printf("%x ", userPlainText[i]);
	}
